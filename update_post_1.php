...
if (isset($_POST['submit'])) 
{
	...
	if (empty($edit_title)) 
	{
		...
		redirect("www.edit_post.php?id=<?php echo $edit_post_id; ?>");
...
<body>
	...
	<div class="container">
		<div class="row">
			<div class="col-md-12 ">
			<h1><i class="fas fa-edit" style="color: #3F628A;"> Edit Post </i></h1>
			</div>
		</div>
	</div>
	</header>
			...
			<div class="offset-lg-1 col-md-10 bg-light" style="min-height:500px;">
				<?php 
					...
					{					
					   ...
					   $sql_edit_post  = "SELECT * FROM post WHERE id = $edit_post_id";
					   $stmt_edit_post = $connect->conn()->query($sql_edit_post);					
					
					   while ($data_row = $stmt_edit_post->fetch()) 
						{
									
							$old_title          = $data_row['title'];
							$old_category       = $data_row['category'];
							$old_image          = $data_row['image'];
							$old_post           = $data_row['post'];				
						}
					}
				 ?>
				<!-- FORM STARTS HERE-->	 
				<form class="" action="www.post.php" method="post" enctype="multipart/form-data">
					<div class="card bg-secondary">
						<div class="card-header">
							<h1 style="color: #3F628A;">Edit Post</h1>
						</div>
				<!-- INSERT NEW TITLE -->
						<div class="card-body bg-dark">
							<div class="form-group">
								<label for="title"><span class="label"> Enter New Title: </span></label>
								<input class="form-control" id="title" type="text" name="edit_title" value="<?php echo $old_title; ?>">
							</div>
				<!-- SELECT NEW CATEGORY -->
								<div class="form-group"><span class="label">Existing Category: <?php echo $old_category; ?></span></div>
								<label for="categoryTitle"><span class="label"> Edit Category: </span></label>
				...
				<!-- SELECT IMAGE INPUT-->								
							<div class="form-group py-2">
								<div><span class="label">Existing Image:  </span><img src="<?php echo 'uploads/'.$old_image; ?>" width="100"></div>
								<label for="image"><span style="color: white;">Select New Image:</span></label>
									<div class="custom-file">	
									<input class="custom-file-input" type="file" name="edit_image" id="image" value="">
									<label class="custom-file-label" for="image"> Select New Image... </label>
									</div>
									</div>	
							</div>
				<!-- POST DESCRIPTION OF TITLE AND CATEGORY -->								
							<div class="form-group bg-dark px-4 py-2">
								<label for="post"><span style="color: white;">Edit Post Description:</span></label>
								<textarea name="postdesciption" class="form-control" id="edit_post" rows="8" cols="80"><?php echo $old_post; ?></textarea>
							</div>
