...
{
	$newPost         = test_input($_POST['newpost']);
	$categoryTitle   = test_input($_POST['categoryTitle']);
	$postdesciption  = test_input($_POST['postdesciption']);
	...
	}else 
	{
		...
		$sql      = "INSERT INTO post(datetime, title, category, author, image, post) VALUES(:New_date_Time, :New_Title, :New_Category, :New_Author, :New_Image, :New_Post)";
		$pre_stmt = $connect->conn()->prepare($sql);
		$pre_stmt->bindValue(':New_date_Time', $date_time);
...
<body>
	...
	<div class="container">
		<div class="row">
			<div class="col-md-12 ">
			<h1><i class="fas fa-edit" style="color: #3F628A;"> Create And Add New Post </i></h1>
			</div>
		</div>
	</div>
	</header>
			...
			<div class="offset-lg-1 col-md-10 bg-light" style="min-height:500px;">
				...
				<!-- POST DESCRIPTION OF TITLE AND CATEGORY -->								
							<div class="form-group bg-dark px-4 py-2">
								<label for="post"><span style="color: white;">Post Description:</span></label>
