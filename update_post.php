...
if (isset($_POST['submit'])) 
{
	...
	}else 
	{
		...
		if (!empty($image)) 
		{
			...
			$edit_sql = "UPDATE post SET title = '$edit_title', category = '$edit_categoryTitle', image = '$image', post = '$edit_post' WHERE id = $search_param";
			$execute = $connect->conn()->query($edit_sql);
			move_uploaded_file($_FILES["edit_image"]["tmp_name"], $target_file);
		}else
		{
			$connect = new conn_cms();
			$edit_sql = "UPDATE post SET title = '$edit_title', category = '$edit_categoryTitle', post = '$edit_post' WHERE id = $search_param";
			$execute = $connect->conn()->query($edit_sql);
